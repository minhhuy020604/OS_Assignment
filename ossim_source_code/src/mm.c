...
{                                         // no guarantee all given pages are mapped
   ...
   while ( pgit < pgnum )
 {
    ...
    pgn = PAGING_PGN ( addr + pgnum * PAGING_PAGESZ );
...
int alloc_pages_range(struct pcb_t *caller, int req_pgnum, struct framephy_struct** frm_lst)
{
    ...
    for (pgit = 0; pgit < req_pgnum; pgit++)
    {
        if (MEMPHY_get_freefp(caller->mram, &fpn) == 0)
        {
            ...
            *temp = *newfp_str;
            temp->fp_next = caller->mram->used_fp_list;
            caller->mram->used_fp_list = temp;
        }
        else
        {
            // ERROR CODE of obtaining some but not enough frames
            int vicfpn, vicpgn, swpfpn;
            uint32_t vicpte;
            ...
            if (MEMPHY_get_freefp(caller->active_mswp, &swpfpn) == 0)
            {
                ...
                {
                    if (mswp + i == caller->active_mswp) break;
                }
            }
            else
            {
...
int init_mm(struct mm_struct *mm, struct pcb_t *caller)
{
    ...
    /* Update vma0 next */
    vma0->vm_next = vma1;  // Assuming vma1 is the next VM area

    /* Update one vma for HEAP */
    vma1->vm_id = 1;  // Assuming ID for heap
   // vma1->vm_start = HEAP_START;  // Define an appropriate start address for the heap
    //vma1->vm_end = HEAP_END;      // Define the end address for the heap
    vma1->sbrk = vma1->vm_start;

    struct vm_rg_struct *first_rg_heap = init_vm_rg(vma1->vm_start, vma1->vm_end, 0);
    enlist_vm_rg_node(&vma1->vm_freerg_list, first_rg_heap);

    /* Set next for vma1 */
    vma1->vm_next = NULL;  // Assuming it's the last one in the list for now
    ...
    /* Update mmap */
    mm->mmap = vma0;  // Start the mmap list with vma0
